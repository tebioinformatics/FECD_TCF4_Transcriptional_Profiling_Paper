...
s2c <- read.table("Import_Sample_List_Case_vs_Control.txt", header=T, sep="\t", stringsAsFactors=F)
files <- s2c$path
...
write.table(txi, file="DataMatrix_ImportSampleList_Case_vs_Control.txt", sep="\t", quote=F)
...
# Write out data
write.table(res_wt_naomit, file="DESeq2_Result_of_WaldTest_Case_vs_Control.txt", sep="\t", quote=F)
